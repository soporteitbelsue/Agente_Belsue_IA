...
/** Trazo de cada icono de las secciones extra (Heroicons, 24px, contorno). */
const EXTRA_LINK_ICONS: Record<ExtraLinkIcon, string> = {
  // Birrete
  cursos:
      ...
      <div className="space-y-2 p-3">
        ...
        {config.extraLinks?.map((link) => (
          <Link
            ...
            href={link.href}
            onClick={() => onNavigate?.()}
            className="flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-600 transition hover:border-belsue/40 hover:text-belsue"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d={EXTRA_LINK_ICONS[link.icon]} />
            </svg>
            {link.label}
          </Link>
        ))}
